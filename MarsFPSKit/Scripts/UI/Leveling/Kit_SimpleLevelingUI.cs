...
namespace MarsFPSKit
{
    public class Kit_SimpleLevelingUI : Kit_LevelingUIBase
    {
        ...
        IEnumerator FadeLevelUp(int newLvl)
        {
            ...
            levelUpText.text = "Level up!";
            //Set image
            levelUpImage.sprite = main.gameInformation.leveling.levelUpIcons[newLvl - 1];
            animationFade.Play("Fade");
            //Wait
            yield return new WaitForSeconds(animationLength);
            //Get unlocked items
            UnlockInformation[] unlockedItems = main.gameInformation.GetUnlockedItemsAtLevel(newLvl);
            //Display them
