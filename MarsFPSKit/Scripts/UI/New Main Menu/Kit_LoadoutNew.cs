...
namespace MarsFPSKit
{
    public class Kit_LoadoutNew : Kit_LoadoutBase
    {
        ...
        public override void Initialize()
        {
            ...
            for (int i = 0; i < allLoadouts.Length; i++)
            {
                ...
                for (int j = 0; j < allLoadouts[id].loadoutWeapons.Length; j++)
                {
                    ...
                    Weapons.Kit_WeaponRenderer wpnRender = game.allWeapons[allLoadouts[id].loadoutWeapons[jd].weaponID].firstPersonPrefab.GetComponent<Weapons.Kit_WeaponRenderer>();
        ...
        public void WeaponSelect(int slot, int id)
        {
            ...
            if (game.allWeapons[id].IsWeaponUnlocked(game))
            {
                Weapons.Kit_WeaponRenderer wpnRender = game.allWeapons[id].firstPersonPrefab.GetComponent<Weapons.Kit_WeaponRenderer>();
                    ...
                    if (wpnRender)
                    {
                        ...
                        else
                        {
                            for (int i = 0; i < allLoadouts[currentlySelectedLoadout].loadoutWeapons[currentSelectedWeaponCategory].attachments.Length; i++)
                            {
                                ...
                                allLoadouts[currentlySelectedLoadout].loadoutWeapons[currentSelectedWeaponCategory].attachments[i] = Mathf.Clamp(allLoadouts[currentlySelectedLoadout].loadoutWeapons[currentSelectedWeaponCategory].attachments[i], 0, wpnRender.attachmentSlots[i].attachments.Length - 1);
                            }
                        }
                    }
                    else
                ...
                if (wpnRender && wpnRender.attachmentSlots.Length > 0)
                {
                    ...
                    for (int i = 0; i < wpnRender.attachmentSlots.Length; i++)
                    {
                        ...
                        //Setup text
                        TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                        if (txt) txt.text = wpnRender.attachmentSlots[catId].name;
                        //Setup Button
                        Button btn = go.GetComponentInChildren<Button>();
        ...
        public void WeaponCustomizationSelectCategory(int slot, int wep, int catId)
        {
            ...
            Weapons.Kit_WeaponRenderer wpnRender = game.allWeapons[wep].firstPersonPrefab.GetComponent<Weapons.Kit_WeaponRenderer>();

            if (wpnRender)
            {
                for (int i = 0; i < wpnRender.attachmentSlots[catId].attachments.Length; i++)
                {
                    int id = i;
                    //Create
                    ...
                    //Setup text
                    TextMeshProUGUI txt = go.GetComponentInChildren<TextMeshProUGUI>();
                    if (txt) txt.text = wpnRender.attachmentSlots[catId].attachments[id].name;
                    //Setup Button
                    Button btn = go.GetComponentInChildren<Button>();
        ...
        {
            if (!weaponPreviewObjectThirdPerson) onlyUpdateCustomization = false;
            if (!weaponPreviewObjectFirstPerson) onlyUpdateCustomization = false;
            ...
            if (onlyUpdateCustomization)
            {
                ...
                if (tpWpnRender)
                {
                    tpWpnRender.SetAttachments(allLoadouts[currentlySelectedLoadout].loadoutWeapons[currentSelectedWeaponCategory].attachments, null, null, null);
                }

                Weapons.Kit_WeaponRenderer fpWpnRender = weaponPreviewObjectFirstPerson.GetComponent<Weapons.Kit_WeaponRenderer>();

                if (fpWpnRender)
                {
                    fpWpnRender.SetAttachments(allLoadouts[currentlySelectedLoadout].loadoutWeapons[currentSelectedWeaponCategory].attachments, null, null);
                }
            }
            else
            {
                ...
                if (wpnRender)
                {
                    ...
                    wpnRender.SetAttachments(allLoadouts[currentlySelectedLoadout].loadoutWeapons[currentSelectedWeaponCategory].attachments, null, null);
                }
                        ...
                        for (int j = 0; j < allLoadouts[id].loadoutWeapons.Length; j++)
                        {
                            ...
                            if (savedWeapon < game.allWeapons.Length)
                            {
                                ...
                                if (game.allWeapons[savedWeapon].weaponName == savedWeaponName)
                                {
                                    ...
                                    Weapons.Kit_WeaponRenderer wpnRender = game.allWeapons[savedWeapon].firstPersonPrefab.GetComponent<Weapons.Kit_WeaponRenderer>();
                                    ...
                                    if (wpnRender)
                                    {
                                        ...
                                        if (savedAttachments.Length == wpnRender.attachmentSlots.Length)
                                        {
                                            ...
                                            {
                                                int od = o;
                                                allLoadouts[id].loadoutWeapons[jd].attachments[od] = Mathf.Clamp(allLoadouts[id].loadoutWeapons[jd].attachments[od], 0, wpnRender.attachmentSlots[od].attachments.Length - 1);
                                            }
                                        }
                                        else
                                        {
